...
public interface ProductDetailRepository extends JpaRepository<ProductDetail, Integer> {
    ...
    @Query(value = "SELECT * FROM product_detail pd WHERE pd.productid = :productId AND pd.price = (SELECT MIN(pd2.price) FROM product_detail pd2 WHERE pd2.productid = :productId) LIMIT 1", nativeQuery = true)
